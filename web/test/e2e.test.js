...
async function main() {
  ...
  check('专注 Tab 与其它 Tab 样式统一（无特殊圆形按钮）',
  ...
  check('设置页不再显示「动态动画」开关（v0.29.0 移除）', await page.locator('input[data-toggle="animations"]').count() === 0);
  check('动画默认开启（html 无 no-anim）', !(await page.evaluate(() => document.documentElement.classList.contains('no-anim'))));
  const animDur = await page.evaluate(() => getComputedStyle(document.querySelector('.progress-pill .bar > i')).animationDuration);
  check('进度条流光动画默认运行', parseFloat(animDur) > 0, String(animDur));
