...
| 组件 | 作用 | 费用 |
| --- | --- | --- |
| GitHub Releases | 存放各平台安装包（APK / exe / dmg / AppImage / HAP / zip），单文件上限 2 GB | 免费，不限带宽 |
| GitHub Pages | 静态托管 `update.json`、macOS `appcast.xml`，避免 API 限流 | 免费 |
...
git push tag v0.26.0
        ...
        ▼
GitHub Actions 并行构建：
  Android APK / Windows exe / macOS dmg / Linux AppImage / Web 产物
        │
        ▼
...
```json
{
  ...
  "platforms": {
    ...
    "macos": {
      "url": "https://github.com/HelloXiYangyang/SugarPaper/releases/download/v0.26.0/SugarPaper-0.26.0-macos-universal.zip",
      "sha256": "g7h8i9..."
    },
    "linux": {
      "url": "https://github.com/HelloXiYangyang/SugarPaper/releases/download/v0.26.0/SugarPaper-0.26.0-linux-x86_64.AppImage",
      "sha256": "j0k1l2..."
    },
    "harmonyos": {
...
| macOS | .zip（内含 .app）或 .dmg | Sparkle 框架自动更新 | 是（$99/年开发者账号，签名 + 公证） |
| Linux | AppImage | 下载新 AppImage 替换旧文件后重启 | 否 |
...
- **标准方案：Sparkle**（macOS 最成熟的自动更新框架，Slack、VS Code 都在用）。
  - `appcast.xml` 放 GitHub Pages，指向 GitHub Release 里的 `.zip`（内含签名后的 .app）。
  - 通过 Swift 平台通道集成到 Flutter 的 `macos/Runner`，Dart 侧定时检查 + 弹更新窗口。
  - **必须**有 Apple Developer 账号（$99/年）做 Developer ID 签名 + 公证（notarization 免费）。否则 Gatekeeper 会拦自动更新，用户只能右键打开。
- **无账号时的降级方案（DIY）**：检测到新版本 → 下载 .zip → 退出应用 → 用辅助 shell 脚本把新 .app 覆盖到 `/Applications` → 重新打开。能用，但体验差、每次都要过 Gatekeeper，只建议作为过渡。
- 结论：macOS 想体验好，$99/年的开发者账号是绕不开的成本（不是服务器成本）。

### 4.4 Linux

- **分发物**：AppImage（免依赖、免 root、双击即用）。
- **更新流程**：下载新 AppImage → 校验 → 覆盖当前文件 → `chmod +x` → 重启应用。Flutter 里用 `Process` 即可，和 Windows 绿色版思路一样。
- **进阶**：集成 AppImageUpdate（zsync 增量更新，只下载差异部分），后续版本再考虑。
- **备选**：上 Flathub（免费），Flatpak 自带自动更新，等于平台帮你管。

### 4.5 iOS

**必须诚实说明：苹果不允许从任意网址安装 IPA，GitHub 分发 iOS 安装包在技术上不可行**（个人侧载仅限测试设备且 7 天过期，需要每台设备 UDID 注册重签）。

- 唯一合理路径：上 **App Store**（或先 TestFlight 内测），苹果自动处理更新，零服务器、零成本（除 $99/年账号）。
- 这是好事：iOS 端你什么都不用做，商店就是自动更新系统。
...
| macOS | Sparkle（平台通道）或 DIY 覆盖脚本 |
| Linux | 覆盖 AppImage → `chmod +x` → 重启 |
...
jobs:
  ...
  linux:      # ubuntu-latest，flutter build linux + appimagetool
  macos:      # macos-latest，flutter build macos + dmg/zip
...
| `APPLE_CERTIFICATE` 等 | macOS/iOS 签名公证 | 阶段 2/3 |
...
1. 买 Apple Developer 账号（$99/年）；
2. 集成 Sparkle（Swift 平台通道 + appcast.xml 放 Pages）；
3. 签名 + 公证，验证自动更新全链路。

### 阶段 3：iOS 与鸿蒙

1. iOS 上 App Store / TestFlight，商店自动更新；
2. 鸿蒙上 AppGallery（若用 OpenHarmony Flutter fork 另行评估）；
3. 可选：Ed25519 签名 update.json，加固防篡改。
