...
const isExternal = computed(() => {
  return !!props.downloadRouteRoot && /^https?:\/\//.test(props.downloadRouteRoot);
});
...
<template>
  ...
  >
    <a
      v-if="mainTo && isExternal"
      class="fluent-split-button__main"
      :href="mainTo"
      target="_blank"
      rel="noopener"
    >
    ...
    <transition name="fluent-flyout">
      <div v-if="isOpen" class="fluent-split-button__dropdown">
        <template v-for="item in menuItems" :key="item.key">
          ...
          <RouterLink v-else-if="item.to" custom :to="item.to" v-slot="{ href, navigate }">
            <a
              class="fluent-split-button__item"
              :href="href"
